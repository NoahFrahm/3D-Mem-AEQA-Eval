...
pred_path = f'data/metrics/{args.filename}'

log_file = []
log_file.append(str(args.filename).replace('gpt_answer-metrics-', '').replace('.json', ''))
log_file.append('\n')

# Use Blind LLM as the baseline for unsuccessful episodes
baseline_path = f'data/{args.dataset}-gpt-4o-1234-metrics.json'
#baseline_path = f'data/open-eqa-184-gpt-4o-1234-metrics.json'
...
def spl(path_length, gt_path_length):
    # print(path_length, gt_path_length, gt_path_length / max(gt_path_length, path_length))
    # raise
...
for category, scores in separate_scores.items():
    ...
    scores = [1.0 if score < 1.0 else score for score in scores]
    ...
    log_file.append(f'{category}: {scores:.2f}\n')
    log_file.append(f'{category} SPL: {spl_scores:.2f}\n')
...
log_file.append(f'Total: {total_scores:.2f}\n')
log_file.append(f'Total SPL: {total_spl:.2f}\n')

dir = os.path.dirname(os.path.abspath(__file__))

with open(os.path.join(dir, 'data', 'log.txt'), "a") as myfile:
